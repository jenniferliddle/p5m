...
var host = getUrlQueryStringValue('host');
var cfg_name = getUrlQueryStringValue('cfg_name');
var pid = getUrlQueryStringValue('pid');
...
function loadFromMonitor() {
	d3.json("/cgi-bin/getConfig")
		.header("Content-Type", "application/x-www-form-urlencoded")
		.post("host="+host+"&pid="+pid+"&cfg_name="+cfg_name, function(error, data) {
...
//	var graph = JSON.parse(stripped_data);
	...
	if (host != '') {
		refresh_progress();
		window.setInterval(refresh_progress, 60000);
	}
}
...
function refresh_progress() {
	...
	d3.json("/cgi-bin/getProgress")
		.header("Content-Type", "application/x-www-form-urlencoded")
		.post("host="+host+"&pid="+pid+"&cfg_name="+cfg_name+"&logdir="+logdir, function(error, json) {
		if (error) return console.warn(error);
		if (json == "") return 0;
				...
				if (job_status == 2) {
					...
					i = setInterval( function() {
						d3.select('#x'+c.name).style('stroke','#000').style('stroke-width','1px')
							.transition().style('stroke','orange')
							.each('end',function() {
								d3.select(this)
